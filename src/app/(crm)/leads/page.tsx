...
import { leadsStageFilters as stageFilters, leadsSources as sources, leadsStages as stages } from '@/lib/mockData';
...
const emptyForm = {
  ...
  source: 'Meta Ads', stage: 'new', leadScore: 50,
  intent: '', whatsapp: false,
};
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, width: 540, maxWidth: '95vw', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.4)', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <input style={inputStyle('name')} value={form.name} placeholder="e.g. Riya Sharma" onChange={e => onChange('name', e.target.value)} />
            {errors.name && <span style={{ fontSize: 11, color: 'var(--rose)' }}>{errors.name}</span>}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label style={labelStyle}>Company</label>
            <input style={inputStyle('company')} value={form.company} placeholder="e.g. BloomAds" onChange={e => onChange('company', e.target.value)} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <input type="range" min={0} max={100} value={form.leadScore} onChange={e => onChange('leadScore', Number(e.target.value))} style={{ flex: 1, accentColor: 'var(--purple)' }} />
              <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', minWidth: 28, textAlign: 'right' }}>{form.leadScore}</span>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, justifyContent: 'center' }}>
            <label style={labelStyle}>WhatsApp Active</label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              <input type="checkbox" checked={form.whatsapp} onChange={e => onChange('whatsapp', e.target.checked)} style={{ width: 16, height: 16, accentColor: 'var(--emerald)', cursor: 'pointer' }} />
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 28, width: 380, maxWidth: '95vw', boxShadow: '0 24px 64px rgba(0,0,0,0.4)', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
          Are you sure you want to delete <strong style={{ color: 'var(--text-primary)' }}>{lead.name}</strong>? This action cannot be undone.
        </div>
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 4, borderTop: '1px solid var(--border)' }}>
          <button onClick={onClose} className="btn btn-ghost">Cancel</button>
...
export default function LeadsPage() {
  ...
  const fetchLeads = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ type: 'lead', limit: '200' });
  ...
  const openEdit = (l: Lead) => {
    setEditLead(l);
    setEditForm({ name: l.name, company: l.company ?? '', phone: l.phone ?? '', email: l.email ?? '', source: l.source ?? 'Meta Ads', stage: l.stage, leadScore: l.leadScore, intent: l.intent ?? '', whatsapp: l.whatsapp });
    setEditErrors({});
  };
  ...
  const handleEdit = async () => {
    ...
    const res = await fetch(`/api/contacts/${editLead!.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async () => {
    await fetch(`/api/contacts/${deleteLead!.id}`, { method: 'DELETE' });
    setDeleteLead(null);
    fetchLeads();
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
        {[
          { label: 'Total Leads', value: leads.length.toLocaleString(), color: 'blue' },
          { label: 'New This Week', value: leads.filter(l => l.stage === 'new').length.toString(), color: 'purple' },
          { label: 'Hot Leads (Score ≥ 80)', value: leads.filter(l => l.leadScore >= 80).length.toString(), color: 'amber' },
          { label: 'Avg. Lead Score', value: (leads.reduce((s, l) => s + l.leadScore, 0) / (leads.length || 1)).toFixed(1), color: 'emerald' },
        ].map(s => (
          ...
          <div className="table-wrap">
            <table>
              ...
              <tbody>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <div style={{ width: 36, height: 4, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}>
                          <div style={{ width: `${l.leadScore}%`, height: '100%', background: l.leadScore >= 80 ? 'var(--emerald)' : l.leadScore >= 60 ? 'var(--amber)' : 'var(--rose)', borderRadius: 2 }} />
                        </div>
                        <span style={{ fontSize: 12, fontWeight: 700, color: l.leadScore >= 80 ? 'var(--emerald)' : l.leadScore >= 60 ? 'var(--amber)' : 'var(--rose)' }}>{l.leadScore}</span>
                      </div>
                    </td>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={11} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>No leads found</td></tr>
                )}
              </tbody>
            </table>
          </div>
